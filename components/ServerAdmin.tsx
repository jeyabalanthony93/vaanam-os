...
import { Server, Settings, Database, Mail, Shield, Download, Play, RefreshCw, Cpu, HardDrive, Globe, DollarSign, BarChart3, Calculator, Award, ToggleRight, Network, Zap } from 'lucide-react';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Legend } from 'recharts';
import { ServerEnvironment } from '../types';

const ServerAdmin: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'DASHBOARD' | 'FINOPS' | 'USAGE'>('DASHBOARD');
      ...
      <div className="flex-1 overflow-y-auto p-8">
          ...
          {activeTab === 'DASHBOARD' && (
              <div className="space-y-8 animate-in fade-in duration-300">
                      ...
                      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
                          ...
                          <div className="mt-6 p-4 bg-indigo-900/20 border border-indigo-500/30 rounded-lg">
                              <p className="text-xs text-indigo-300">
                                  <strong>AI Optimization:</strong> Resources are automatically scaled based on agent workload. No manual intervention required.
                              </p>
                          </div>
                      </div>
                  </div>
                  ...
                  {/* App Installer */}
                  <div>
                      <h3 className="text-xl font-bold text-white mb-4">Open Source Installer</h3>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                          {[
                              { name: 'WordPress', desc: 'CMS Platform', icon: Globe },
                              { name: 'GitLab CE', desc: 'DevOps Lifecycle', icon: Settings },
                              { name: 'Nextcloud', desc: 'File Share', icon: HardDrive },
                              { name: 'JupyterHub', desc: 'Data Science', icon: Database },
                          ].map((app) => (
                              <div key={app.name} className="bg-slate-800/40 p-4 rounded-xl border border-slate-800 hover:bg-slate-800 transition group cursor-pointer">
                                  <div className="flex justify-between items-start mb-2">
                                      <div className="p-2 bg-slate-700 rounded-lg text-slate-300 group-hover:bg-cyan-500 group-hover:text-white transition">
                                          <app.icon size={20} />
                                      </div>
