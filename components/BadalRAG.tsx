...
import { Database, Search, Layers, Settings, FileText, UploadCloud, RefreshCw, Shield, Play, Plus, Trash2, Cpu, Activity, Zap, Server, Lock, ExternalLink, ArrowRight } from 'lucide-react';
import { KnowledgeBase, RAGSearchResult } from '../types';
import { simulateHybridSearch, simulateRAGIngestion } from '../services/geminiService';

const BadalRAG: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'OVERVIEW' | 'PIPELINES' | 'VECTOR_DB' | 'PLAYGROUND' | 'GOVERNANCE'>('OVERVIEW');
          ...
          <div className="flex bg-slate-800 rounded-lg p-1 border border-slate-700">
              {[
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 px-4 py-2 rounded-md text-xs font-bold transition ${activeTab === tab.id ? 'bg-emerald-600 text-white shadow' : 'text-slate-400 hover:text-white hover:bg-slate-700'}`}
                  >
